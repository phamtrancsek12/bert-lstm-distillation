...
def load_distill_data(text_field, label_field_0, label_field_1, label_field_2):
    logger.info("Load training data....")
    # Load data from csv file
    # For train data, labels are logits output from teacher model
    # For test data, labels are binary (0/1)
    train, test = data.TabularDataset.splits(path=DATA_PATH, train=TRAIN_FILE, test=TEST_FILE, format='csv', skip_header=True,
                                                  fields=[('text', text_field), ('label_0', label_field_0),
                                                          ('label_1', label_field_1), ('label_2', label_field_2)])
